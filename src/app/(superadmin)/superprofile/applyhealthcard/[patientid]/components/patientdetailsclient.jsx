...
const UserProfile = ({ userdata }) => {
  ...
  const [approvalStatus, setApprovalStatus] = useState(
    userdata.healthcard[0]?.approvalStatus || "Pending"
  );
...
const issues = [
  ...
  const handleSubmitRemark = async () => {
    ...
    try {
      ...
      // Fetch the updated approval status and set it to state
      const updatedApprovalStatus = updatedCard.approvalStatus;
      setApprovalStatus(updatedApprovalStatus);

      // Update the userdata with the new approval status
      const updatedUserData = {
        ...userdata,
        healthcard: [
          { ...userdata.healthcard[0], approvalStatus: updatedApprovalStatus },
        ],
      };
        ...
        <div className="p-6">
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
      <h3 className="font-medium text-gray-600">Form Document</h3>
      {userdata.form ? (
        <Docviewer userdata={userdata.form} />
      ) : (
        <p className="text-[12px]">No form document available.</p>
      )}
    </div>
            </div>
          </div>
          ...
          {userdata?.healthcard?.[0]?.approvalStatus === "APPROVED" ? (
  <>
...
) : userdata?.healthcard?.[0]?.approvalStatus === "REJECTED" ? (
  <>
  ...
  <>
    <button
      ...
      onClick={() => openModal("approve", userdata.id)}
    >
      Approve
    </button>
    <button
      className="bg-red-600 text-white px-6 py-2 rounded-xl"
      onClick={() => openModal("reject", userdata.id)}
    >
      Reject
    </button>
  </>
