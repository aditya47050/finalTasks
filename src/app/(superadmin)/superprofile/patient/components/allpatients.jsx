...
const AllPatientsdata = ({ userdata }) => {
  ...
  const filteredData = useMemo(() => {
    return userdata.filter((user) => {
      ...
      return (
        ...
        (!filters.district ||
          user.district?.toLowerCase() === filters.district.toLowerCase()) &&
        (!filters.taluka ||
          user.taluka?.toLowerCase() === filters.taluka.toLowerCase()) &&
        (!filters.hasPanCard ||
          user.hasPanCard?.toString() === filters.hasPanCard) &&
        (!filters.income || user.income?.toString() === filters.income) &&
        (!filters.healthInsurance ||
          user.healthInsurance?.toString() === filters.healthInsurance) &&
        (!filters.abhacard || user.abhacard?.toString() === filters.abhacard) &&
        (!filters.ayushmancard ||
          user.ayushmancard?.toString() === filters.ayushmancard) &&
        (!filters.rationcard ||
          user.rationcard?.toString() === filters.rationcard) &&
        (!filters.organDonation ||
          user.organDonation?.toString() === filters.organDonation) &&
        (!filters.isCompanyRegistered ||
          user.isCompanyRegistered?.toString() ===
            filters.isCompanyRegistered) &&
        (!filters.bankName ||
          user.bankName
            ?.toLowerCase()
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        header: "Payment",
        cell: ({ row }) => {
          const payments = row.original.Payment;
          if (!payments || payments.length === 0) return "No Payment";

          // Get the latest payment by createdAt
          ...
          return latestPayment.paymentStatus;
        },
      },
  ...
  return (
    <div className="md:container shadow-xl mx-auto font-poppins">
      <div className="overflow-x-auto xs:px-2 min-[1000px]:px-0 min-[1100px]:px-[32px] container mx-auto min-[1000px]:w-[700px] min-[1100px]:w-[900px] xl:w-[1000px] xlg:w-[1200px] w-[350px] md:w-[700px] bg-white">
        <HeadingClientMain main={"All Patients List"} sub={"Full Details"} />
