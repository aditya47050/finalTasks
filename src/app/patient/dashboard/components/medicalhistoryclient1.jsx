...
import { X, Plus, Calendar, Stethoscope, FileText, Activity, Heart, Brain, Ruler, Thermometer } from "lucide-react";
...
const MedicalHistoryPage = ({ userdata, existingHistory = [] }) => {
  ...
  const [bodyInfo, setBodyInfo] = useState({
    bpValue: userdata?.medicalhistory?.bpvalue || "",
    weight: userdata?.medicalhistory?.weight || "",
    height: userdata?.medicalhistory?.height || "",
    temperature: userdata?.medicalhistory?.temperature || "",
    pulseRate: userdata?.medicalhistory?.pulseRate || "",
  });
  ...
  const [isBodyInfoSaving, setIsBodyInfoSaving] = useState(false);
  ...
  const handleBodyInfoSubmit = async () => {
    setIsBodyInfoSaving(true); // Start loading
    try {
      const response = await fetch(`/api/patient/${userdata.id}/body-info`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        toast.success("Body info saved successfully");
        setShowBodyInfoModal(false);
      } else {
        const errorData = await response.json();
        toast.error(errorData.message || "Failed to save body info");
      }
    } catch (error) {
      console.error("Error:", error);
      toast.error("Error saving body info");
    } finally {
      setIsBodyInfoSaving(false); // End loading
    }
  };
  ...
  <div className="flex justify-between items-center py-4">
    ...
    <div className="flex items-center gap-8">
      <div className="flex items-center gap-2">
        ...
        <span>BP Value: {bodyInfo.bpValue}</span>
      </div>
      <div className="flex items-center gap-2">
        <Activity className="w-5 h-5 text-green-600" />
        <span>Weight: {bodyInfo.weight}</span>
      </div>
      <div className="flex items-center gap-2">
        <Ruler className="w-5 h-5 text-blue-600" />
        <span>Height: {bodyInfo.height}</span>
      </div>
      <div className="flex items-center gap-2">
        <Thermometer className="w-5 h-5 text-yellow-600" />
        <span>Temp: {bodyInfo.temperature}</span>
      </div>
      <div className="flex items-center gap-2">
        <Activity className="w-5 h-5 text-purple-600" />
        <span>Pulse Rate: {bodyInfo.pulseRate}</span>
      </div>
    </div>
    {/* Buttons */}
    <div className="flex gap-1">
      ...
      <Button
        onClick={() => setShowBodyInfoModal(true)}
        className="bg-blue-500 text-white rounded-xl hover:bg-blue-600 flex items-center gap-2 ml-4"
      >
        Body Info
      </Button>
    </div>
          ...
          <div className="bg-white rounded-xl max-w-6xl w-full max-h-[70vh] overflow-y-auto">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-center gap-4 rounde-xl">
                ...
                <Button
                  onClick={handleBodyInfoSubmit}
                  className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 rounded-xl"
                  disabled={isBodyInfoSaving} // Disable button while loading
                >
                  {isBodyInfoSaving ? (
                    <>
      ...
      {showBodyInfoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Body Info</h3>
              <button onClick={() => setShowBodyInfoModal(false)} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">BP Value</label>
                <input
                  type="text"
                  value={bodyInfo.bpValue}
                  onChange={(e) => setBodyInfo({ ...bodyInfo, bpValue: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setBodyInfo({ ...bodyInfo, weight: e.target.value })}
                  className="w-full p-2 border border-gray-300 rounded-lg"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setBodyInfo({ ...bodyInfo, height: e.target.value })}
                  className="w-full p-2 border border-gray-300 rounded-lg"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={bodyInfo.temperature}
                  onChange={(e) => setBodyInfo({ ...bodyInfo, temperature: e.target.value })}
                  className="w-full p-2 border border-gray-300 rounded-lg"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={bodyInfo.pulseRate}
                  onChange={(e) => setBodyInfo({ ...bodyInfo, pulseRate: e.target.value })}
                  className="w-full p-2 border border-gray-300 rounded-lg"
                />
              </div>
            </div>
            <div className="flex justify-end space-x-3 mt-6">
              <Button onClick={() => setShowBodyInfoModal(false)} variant="outline" className="px-4 py-2 text-sm bg-transparent rounded-xl">
                Cancel
              </Button>
              <Button onClick={handleBodyInfoSubmit} className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 rounded-xl">
                Save
              </Button>
            </div>
          </div>
