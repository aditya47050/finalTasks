...
const DigitalHealthCardPage = async () => {
  ...
  const data = await db.Ambulance.findFirst({
    ...
    include: {
      AmbulanceCertificate: true, // Fetch certificates
      reviews: {     // ✅ correct field
      include: {
        patient: {
          select: {
            id: true,
  ...
  if (!data || !data.AmbulanceCertificate.length) {
    return (
  ...
  const certificate = data.AmbulanceCertificate[0];
