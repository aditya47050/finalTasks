...
export default function AdRequestsManager({user}) {
  ...
  const { data = {}, isLoading, mutate } = useSWR(`/api/ambulance/ad-requests?${qs}`, fetcher)
  ...
  const onDelete = useCallback(
    async (row) => {
      ...
      await fetch(`/api/ambulance/ad-requests/${row.id}`, { method: "DELETE" })
      mutate()
    },
    [mutate]
  )
  ...
  const onExport = useCallback(async () => {
    ...
    if (query.search) p.set("search", query.search)
    if (query.status && query.status !== "ALL") p.set("status", query.status)
    const res = await fetch(`/api/ambulance/ad-requests?${p.toString()}`)
    ...
    >
      <Card className="bg-white shadow-lg rounded-xl animate-in fade-in-50">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="sm:max-w-xl data-[state=open]:animate-in data-[state=open]:fade-in-50">
            <DialogHeader>
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        initialData?.id ? `/api/ambulance/ad-requests/${initialData.id}` : `/api/ambulance/ad-requests`,
        {
