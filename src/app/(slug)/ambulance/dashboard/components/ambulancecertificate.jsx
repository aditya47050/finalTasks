...
export default function Certificate(data) {
  const ambulanceCertificate = data?.data?.AmbulanceCertificate?.[0]; // first certificate
  const createdAtRaw = ambulanceCertificate?.createdAt;
  const regno = data?.data?.AmbulanceCertificate?.[0].cardNo;
...
const avgRating =
  ...
  for (let i = 1; i <= 5; i++) {
    if (avgRating >= i) {
      // full star
      stars.push(<Star key={i} className="text-yellow-500 fill-yellow-500 w-6 h-6" />)
    } else if (avgRating >= i - 0.5) {
      // half star
      stars.push(<StarHalf key={i} className="text-yellow-500 fill-yellow-500 w-6 h-6" />)
    } else {
      // empty star
      stars.push(<Star key={i} className="text-white w-6 h-6" />)
    }
  }
    const doctorName = data?.data?.ownerfirstname && data?.data?.ownerlastname
    ? `${data?.data.ownerfirstname} ${data?.data.ownermiddlename} ${data?.data.ownerlastname}`
    : data?.data?.firstName || data?.data?.lastName || "Ambulance Name";
  ...
  return (
    <div className="flex flex-col items-center p-4 md:p-8">
                ...
                <div className="bg-[#3b68b5] text-white p-6 flex flex-col items-center min-h-[66%] ">
                  ...
                  <div className="flex items-center justify-between w-full mt-8">
                    ...
                    <div>
                      ...
                      <div className="text-xl font-bold mt-2">{regno}</div>
                    </div>
            ...
            <div>
              ...
              <div className="text-xl font-bold mt-2 text-center">{avgRating.toFixed(1)}</div>
            </div>
