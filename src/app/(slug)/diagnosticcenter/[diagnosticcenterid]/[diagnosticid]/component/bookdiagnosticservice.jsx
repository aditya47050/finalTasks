...
const BookDiagnosticDialog = ({ open, onOpenChange, patientId, serviceId }) => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/diagnostic-center/bookservices", {
        method: "POST",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg bg-white p-6 rounded-xl shadow-xl">
        <DialogHeader className="text-center pb-4">
          <HeadingClientMain main={"  Book Diagnostic Service"} />
        </DialogHeader>
