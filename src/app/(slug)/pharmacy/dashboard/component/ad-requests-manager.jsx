...
export default function AdRequestsManager({user}) {
  ...
  const { data = {}, isLoading, mutate } = useSWR(`/api/pharmacy/ad-requests?${qs}`, fetcher)
  ...
  const onDelete = useCallback(
    async (row) => {
      ...
      await fetch(`/api/pharmacy/ad-requests/${row.id}`, { method: "DELETE" })
      mutate()
    },
    [mutate]
  )
  ...
  const onExport = useCallback(async () => {
    ...
    if (query.search) p.set("search", query.search)
    if (query.status && query.status !== "ALL") p.set("status", query.status)
    const res = await fetch(`/api/pharmacy/ad-requests?${p.toString()}`)
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        initialData?.id ? `/api/pharmacy/ad-requests/${initialData.id}` : `/api/pharmacy/ad-requests`,
        {
