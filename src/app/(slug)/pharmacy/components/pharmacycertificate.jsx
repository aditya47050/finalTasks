...
export default function Certificate(data) {
  const pharmacyCertificate = data?.data?.PharmacyCertificate?.[0];
  const createdAtRaw = pharmacyCertificate?.createdAt;
  const regno = data?.data?.PharmacyCertificate?.[0].cardNo;
  ...
  // 🔹 FIX: Since Pharmacy model doesn't have reviews, use empty array
  const reviews = [];
  const avgRating = 0; // Default rating since no reviews
  ...
  const pharmacyName = data?.data?.regname || "Pharmacy Name";
