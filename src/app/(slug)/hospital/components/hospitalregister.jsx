...
const HospitalRegisterClient = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const registerRes = await fetch("/api/hospital/register", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const responseData = await registerRes.json(); // Parse response JSON

      if (!registerRes.ok) {
        throw new Error(responseData.message || "Failed to create an account");
      }
...
export default HospitalRegisterClient;
