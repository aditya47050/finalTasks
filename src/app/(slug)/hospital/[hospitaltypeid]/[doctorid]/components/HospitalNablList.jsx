...
import { Beaker, CheckCircle, AlertCircle, X } from "lucide-react";
...
export default function HospitalNablPathologyList({ hospitalId, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/hospital/${hospitalId}/nabl-pathology`);
        const result = await res.json();
        if (result.success) setData(result.nablPathology);
      } catch (err) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/90 text-sm mt-1">
                  Laboratory accreditation details and certified services
                </p>
              </div>
            </div>
        ...
        <div className="p-6 sm:p-8 bg-white">
          {loading ? (
            ...
            <div className="space-y-6">
              <Card className={`border ${unified.borderColor} shadow-md rounded-2xl`}>
                <CardContent className="p-6 space-y-3">
                  ...
                  <p className="text-gray-700">
                    <strong>Certificate No:</strong> {data.certificationNumber}
                  </p>
                  <p className="text-gray-700">
                    <strong>Valid Till:</strong> {data.validTill}
                  </p>
                  <p className="text-gray-700">
                    <strong>Lab Manager:</strong> {data.labManager}
                  </p>
                </CardContent>
              </Card>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {data.services.map((s, i) => (
                  <Card key={i} className={`border ${unified.borderColor} rounded-2xl`}>
            ...
            <Card className="p-10 text-center text-gray-600 border">
              ...
              No NABL certification data found for this hospital.
            </Card>
          )}
        </div>
      </DialogContent>
