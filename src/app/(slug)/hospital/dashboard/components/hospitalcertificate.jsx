...
export default function Certificate(data) {
  ...
  const hospitalCertificate = data?.data?.HospitalCertificate?.[0]; // first certificate
const createdAtRaw = hospitalCertificate?.createdAt;

let registrationDate = "N/A";
let renewalDate = "N/A";
const reviews = data?.data?.reviews || [];
const regno = data?.data?.HospitalCertificate?.[0].cardNo;
const averageRating =
  reviews.length > 0
...
if (createdAtRaw) {
  ...
  const doctorName = data?.data?.hspInfo?.regname
    ? `${data?.data?.hspInfo?.regname}`
    :  "Hospital Name";
