...
const DiagnosticCenterApprovalClient = ({ userdata, associations, state = [], dist = [], subdist = [] }) => {
  // associations: array of HospitalDiagnosticCenter records, each must include .id and .hospital (with hspcontact/hspInfo)
...
if (filters.subdistrict !== "all" && h.hspcontact?.subdistrictId !== filters.subdistrict) return false;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `HospitalAssociations_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };

  // handle approval/rejection — NOTE: this calls the API route defined below
  const handleApproval = async (assocId, approved) => {
    try {
      ...
      // IMPORTANT: API path includes diagnostic center id (userdata.id)
      const res = await fetch(`/api/diagnostic-center/${userdata.id}/hospitalapproval`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          hospitalDiagnosticCenterId: assocId,
          approved,
          notes,
        }),
      });
      ...
      // cleanup
      setRemarks((p) => {
        const cp = { ...p }; delete cp[assocId]; return cp;
      });
      setSelectedIssues((p) => {
        const cp = { ...p }; delete cp[assocId]; return cp;
      });
...
<select
  ...
  className="p-2 border rounded-xl"
  disabled={filters.state === "all"} // ✅ disable until state is chosen
>
...
<select
  ...
  className="p-2 border rounded-xl"
  disabled={filters.district === "all"} // ✅ disable until district is chosen
>
...
export default DiagnosticCenterApprovalClient;
