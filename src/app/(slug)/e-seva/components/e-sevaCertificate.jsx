...
export default function Certificate({data,parentEseva}) {
  const doctorCertificate =
    data?.ESevaCertificate?.[0] || data?.ESevaSubAdminCertificate?.[0]; // first certificate
const createdAtRaw = doctorCertificate?.createdAt;
let registrationDate = "N/A";
let renewalDate = "N/A";
const rating = data?.data?.reviews || "5";
  const stars = []
  for (let i = 1; i <= 5; i++) {
    if (rating >= i) {
      // full star
      ...
      // empty star
      stars.push(<StarOff key={i} className="text-white w-6 h-6" />)
    }
  }
  const regno = data?.ESevaCertificate?.[0].cardNo || data?.ESevaSubAdminCertificate?.[0].cardNo;
if (createdAtRaw) {
  ...
  const displayName =
  data?.role === "SubAdmin"
    ? `${data?.name} (SubAdmin of ${data?.parentEseva?.name || "E-Center"})`
    : data?.name || "E-Center Name";
        ...
        <div className="bg-[#3b68b5] text-white p-6 flex flex-col items-center min-h-[66%] ">
          ...
          <div className="flex items-center justify-between w-full mt-8">
            ...
            <div>
              ...
              <div className="text-xl font-bold mt-2">{regno || "RG133ERJTS12"}</div>
            </div>
            ...
            <div>
              ...
              <div className="text-xl font-bold mt-2 text-center">{rating}</div>
            </div>
