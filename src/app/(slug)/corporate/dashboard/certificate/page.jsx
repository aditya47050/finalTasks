...
import Certificate from "../component/corporatecertificate";

const DigitalHealthCardPage = async () => {
  const corporateUser = await getSession();
  ...
  // 🔹 Fetch corporate data and include certificates
  const data = await db.corporate.findFirst({
    where: { email: corporateUser.email },
    include: {
      CorporateCertificate: true,
    },
  });

  // 🔹 If no corporate data found
  if (!data || !data.CorporateCertificate.length) {
    return (
  ...
  const certificate = data.CorporateCertificate[0];
  ...
  if (certificate.approvalStatus === "REJECTED") {
    return (
      <div className="flex flex-col items-center justify-center p-6 bg-white border border-red-200 mx-auto container rounded-[15px] shadow-md mt-6">
        ...
        <div className="mt-6">
          <Link href="/corporate/dashboard/profile">
