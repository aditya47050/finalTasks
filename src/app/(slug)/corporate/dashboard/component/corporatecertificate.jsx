...
export default function Certificate(data) {
  const corporateCertificate = data?.data?.CorporateCertificate?.[0];
  const createdAtRaw = corporateCertificate?.createdAt;
  const regno = data?.data?.CorporateCertificate?.[0].cardNo;
  ...
  // 🔹 Corporate doesn't have reviews, use default rating
  const avgRating = 0;
  ...
  const corporateName = data?.data?.companyName || "Corporate Name";
