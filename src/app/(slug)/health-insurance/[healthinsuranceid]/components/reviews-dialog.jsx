...
export default function HealthInsuranceReviewsDialog({ insuranceid, patientId }) {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const res = await fetch(`/api/health-insurance/${insuranceid}/review`);
              ...
              <div key={r.id} className="border rounded-lg p-3">
                <div className="flex items-center gap-2">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${i < r.rating ? "text-amber-400 fill-amber-400" : "text-gray-300"}`}
                    />
                  ))}
