...
export default function ReviewsDialog({ doctorId, patientId }) {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const res = await fetch(`/api/doctor/${doctorId}/review`)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
          <Star className="text-white md:h-6 md:w-6 h-5 w-5" />
      </DialogTrigger>
            ...
            reviews.map((r) => (
              <div key={r.id} className="border rounded-lg p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">by {r.patient?.name || "Anonymous"}</span>
                </div>
