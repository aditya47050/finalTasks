...
import Certificate from "../components/photographercertificate";
...
const DigitalHealthCardPage = async () => {
  ...
  const data = await db.photographer.findFirst({
    ...
    include: {
      PhotographerCertificate: true, // Fetch certificates
    },
  });
  console.log(data);

  // 🔹 If no doctor data found
  if (!data || !data.PhotographerCertificate.length) {
    return (
  ...
  const certificate = data.PhotographerCertificate[0];
