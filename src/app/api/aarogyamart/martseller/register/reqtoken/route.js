...
export async function POST(request) {
  ...
  const user = await db.MartSeller.findUnique({ where: { email } });
  if (!user) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/martseller/authforgot/forgotpassword?token=${resetToken}`;
