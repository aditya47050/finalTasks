...
export async function POST(request, params) {
  try {
    ...
    const resp = await fetch(
      `${apiUrl}/api/aarogyamart/martseller/auth/?apikey=479693736527271&email=${email}&password=${password}&mobile=${mobile}`
    );
    ...
    if (data.status !== "APPROVED") {
      return new NextResponse(
        JSON.stringify({ msg: "Your account is not approved by admin. Please wait for approval." }),
        { status: 403 }
      );
    }
