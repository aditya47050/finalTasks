...
export async function POST(request) {
  try {
    const { email, name, type  } = await request.json(); // Add type with default value
    ...
    if (!user) {
      return new Response(
        JSON.stringify({ success: false, message: `${type.charAt(0).toUpperCase() + type.slice(1)} not found!` }),
        { status: 404 }
      );
    }
