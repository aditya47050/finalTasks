...
export async function POST(request) {
  ...
  const user = await db.JObUser.findUnique({ where: { email } });
  if (!user) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/jobaadhar/job-seeker/authforgot/forgotpassword?token=${resetToken}`;
