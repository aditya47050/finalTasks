...
export async function POST(request) {
  ...
  const user = await db.Eseva.findUnique({ where: { email } });
  if (!user) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/e-seva/authforgot/forgotpassword?token=${resetToken}`;
