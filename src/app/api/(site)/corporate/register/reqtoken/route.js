...
export async function POST(request) {
  ...
  const user = await db.Corporate.findUnique({ where: { email } });
  if (!user) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/corporate/authforgot/forgotpassword?token=${resetToken}`;
