...
export async function POST(request) {
  ...
  const user = await db.Ambulance.findUnique({ where: { email } });
  if (!user) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/ambulance/authforgot/forgotpassword?token=${resetToken}`;
