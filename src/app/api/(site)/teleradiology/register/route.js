...
export async function POST(request) {
  ...
  const existingUser = await db.TeleRadiology.findUnique({ where: { email } });
  if (existingUser) {
  ...
  const user = await db.TeleRadiology.create({
    data: { mobile, email, pincode, password: hashedPassword },
  });

  return NextResponse.json(
    { success: true, message: "New TeleRadiology Account created", user },
    { status: 201 }
  );
}
