...
export async function PUT(req) {
  try {
    ...
    const existing = await db.bookDiagnosticService.findMany({
      where: {
        preferredDate: {
    ...
    // Update the booking
    const updated = await db.bookDiagnosticService.update({
      where: { id: bookingId },
      data: {
...
async function sendConfirmationEmail(email, { name, serviceName, date, time }) {
  ...
  const htmlContent = `
    ...
    <p>Dear ${name},</p>
    <p>Your diagnostic service appointment has been successfully rescheduled.</p>
