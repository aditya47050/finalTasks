...
export async function POST(req, {params}) {
  ...
  try {
    ...
    const { patientId, serviceId, preferredDate, preferredTimeSlot, notes ,hspRole } =
      body;

  const loggedIn = await getLoggedInUser();
  

    if (!patientId || !serviceId || !preferredDate || !preferredTimeSlot || !hspRole) {
    ...
    // Save booking anyway
    const booking = await db.BookSurgeryTreatment.create({
  data: {
    patientId,
    serviceId,
    preferredDate: new Date(preferredDate + "T00:00:00"), // normalize to local midnight
    preferredTimeSlot,
    notes,
    status: "PENDING", // optional since default is PENDING
  },
});
...
async function sendConfirmationEmail(
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f9f9f9;">
      ...
      <div style="padding: 20px 0;">
        ...
        <p style="font-size: 16px;">Your Surgery and Treatment service has been successfully booked.</p>
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: "Surgery Treatment Service Booking Confirmation",
    html: htmlContent,
  });
}
...
async function sendUnavailableEmail(
  email,
  { bookingId, patientId, serviceId, date, availableSlots,hospitalid }
) {
  const transporter = nodemailer.createTransport({
    service: "Gmail",
    auth: {
  ...
  const slotsHtml = availableSlots
    .map(
      ...
      <a href="${baseUrl}/hospital/reschedule-confirmation?bookingId=${bookingId}&patientId=${patientId}&serviceId=${serviceId}&preferredDate=${date}&preferredTimeSlot=${slot}&hospitalid=${hospitalid}"
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f9f9f9;">
      ...
      <div style="padding: 20px 0;">
        <p style="font-size: 16px;">Dear Patient,</p>
        <p style="font-size: 16px;">You requested a service on <strong>${formattedDate}</strong>, but the selected time is already taken.</p>
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: "Surgery and Treatment Service Slot Unavailable - Please Reschedule",
    html: htmlContent,
  });
}