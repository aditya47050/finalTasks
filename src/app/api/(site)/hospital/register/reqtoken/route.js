...
export async function POST(request) {
  ...
  const user = await db.Hospital.findUnique({ where: { email } });
  if (!user) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/hospital/authforgot/forgotpassword?token=${resetToken}`;
