...
export async function POST(req) {
  try {
    ...
    const { patientId, serviceId, preferredDate, preferredTimeSlot, notes } =
      body;

    if (!patientId || !serviceId || !preferredDate || !preferredTimeSlot) {
    ...
    const bookings = await db.bookDiagnosticService.findMany({
      where: {
        preferredDate: {
    ...
    // Save booking anyway
    const booking = await db.bookDiagnosticService.create({
      data: {
        patientId,
        serviceId,
...
async function sendConfirmationEmail(
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f9f9f9;">
      ...
      <div style="padding: 20px 0;">
        ...
        <p style="font-size: 16px;">Your diagnostic service has been successfully booked.</p>
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: "Diagnostic Service Booking Confirmation",
    html: htmlContent,
  });
}
...
async function sendUnavailableEmail(
  ...
  const slotsHtml = availableSlots
    .map(
      ...
      <a href="${baseUrl}/diagnosticcenter/reschedule-confirmation?bookingId=${bookingId}&patientId=${patientId}&serviceId=${serviceId}&preferredDate=${date}&preferredTimeSlot=${slot}"
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f9f9f9;">
      ...
      <div style="padding: 20px 0;">
        <p style="font-size: 16px;">Dear Patient,</p>
        <p style="font-size: 16px;">You requested a diagnostic service on <strong>${formattedDate}</strong>, but the selected time is already taken.</p>
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: "Diagnostic Service Slot Unavailable - Please Reschedule",
    html: htmlContent,
  });
}
