...
export function CategoryGrid() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const categoriesWithGradients = data.slice(0,6).map((cat) => ({
        ...cat,
      ...
      <section className="pt-6">
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center pt-6">
            <Button className="bg-blue-500 rounded-xl text-white hover:bg-blue-600" disabled>
              Loading...
            </Button>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 min-[1100px]:grid-cols-6 gap-4 md:gap-6">
          {categories.map((category) => (
            <Link key={category.id} href={`aarogyamart/category/${category.id}`}>
        ...
        <div className="flex justify-center pt-6">
          <Link href={"/aarogyamart/category"}>
            <Button className="bg-blue-500 rounded-xl text-white hover:bg-blue-600">
              View More Category
            </Button>
          </Link>
        </div>
